...
%% User promt
global A  B C D K SSS Ts tf AIneq BIneq Aeq Beq LB UB OtherId systemType
Ts = 0.06;  % sampling time period
...
systemType = input('Is this a single system or multi-agent system? 1 for single, 2 for multi agent.\n');
if systemType == 2
    OtherId = input('Please specify the index of the states which belong to other agents\n');
elseif systemType == 1
    OtherId = 0;
elseif systemType ~= 1 && systemType ~= 2
    systemType = input('Undefined system type, try again\n');
end
...
% 
% safetyId = input('Specify the safety index from the state vector\n');
% % safety = SSS(:, safetyId);
% 
% count = 0;
% show = true;
...
% for i = 1 : size(SSS, 1)
% 
%     fprintf(strcat(int2str(i), '\n'));
%     percentage = roundn((i / size(SSS, 1)) * 100, 0);
%     if mod(percentage, 1) == 0 && show == true
%         show = false;
%         fprintf(strcat('progress for S_sigma:', int2str(percentage) , '%%\n'));
%     elseif mod(percentage, 1) ~= 0 && show == false
%         show = true;
%     end
% 
%     initState = SSS(i, :)';
%     [initU, old_e, E] = updateControlActuator(initState,0, initState(3) - 40, 0);
%     u = initU;
%     oldState = initState;
%     fail = false;
%     for t = 0 : Ts : tf
%         newState = updateState(oldState,u);
%         [u, old_e, E] = updateControlActuator(oldState,u, old_e, E);
%         oldState = newState;
%         
%         newState = roundn(newState, granularity);
%         if size(intersect(newState(safetyId)', SSS(:, safetyId), 'rows'), 1) == 0
%             fail = true;
%             break
%         end
% 
...
for i = 1 : size(S_sigma, 1)
    ...
    newStateUB = updateState(oldState, UB);
    newStateLB = updateState(oldState, LB);

    newStateUB = roundn(newStateUB, granularity);
    newStateLB = roundn(newStateLB, granularity);
    if size(intersect(newStateUB(stateOfInterests)', S_sigma(:, stateOfInterests), 'rows'), 1) ~= 0 && size(intersect(newStateLB(stateOfInterests)', S_sigma(:, stateOfInterests), 'rows'), 1) ~= 0
        count = count + 1;
        S_1(count, :) = S_sigma(i, :);
    end
end
