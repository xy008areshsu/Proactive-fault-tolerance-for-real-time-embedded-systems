...
for i = -theta :step: theta
    for j = -theta_dot : step : theta_dot
        ...
        if size(find(abs(x1(:, 3)) > theta), 1) == 0
            ...
            if size(find(abs(x(:, 3)) > theta), 1) == 0
                S_3_theta = [S_3_theta i];
                S_3_theta_dot = [S_3_theta_dot j];
            end
        end
    end
end
...
S_1_theta = [];
S_1_theta_dot = [];
for i = 1 : size(S_3_theta, 2)
    x0 = [0; 0; S_3_theta(i); S_3_theta_dot(i)];
    [y, t, x] = lsim(sys_cl_wrong, r_N, 0: Ts : 2 * Ts, x0);
    if size(find(abs(y(:, 2)) > theta), 1) == 0
        S_1_theta = [S_1_theta S_3_theta(i)];
        S_1_theta_dot = [S_1_theta_dot S_3_theta_dot(i)];
    end  
end
...
for i = 1 : size(S_3_theta, 2)
    ...
    if size(find(abs(y(:, 2)) > theta), 1) == 0
        S_2_theta = [S_2_theta S_3_theta(i)];
        S_2_theta_dot = [S_2_theta_dot S_3_theta_dot(i)];
    end  
end
...
saveas(gcf, strcat('ControlPeriod', num2str(Ts * 1000), 'ms') , 'pdf') %Save figure
